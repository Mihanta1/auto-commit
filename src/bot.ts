...
async function main() {
  ...
  if (tsResult.toString().includes("error")) {
      console.error(
          "Error: TypeScript code contains errors. Please fix the errors before committing."
      );
      return;
  }
...
*/
  ...
  if (modificationsCount < 1) {
    console.error(
      "Error: Number of modifications is less than 10. Please make at least 1 modifications before committing."
    );
    return;
  }
